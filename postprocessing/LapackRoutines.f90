...
interface vector_mul_old

module procedure z_vector_mul_old, c_vector_mul_old
...
subroutine z_vector_mul_old(A,C,z_factor)

 complex(8), intent(in)    :: A(:,:)
 complex(8), intent(inout) :: C(:,:)
 complex(8), intent(in), optional :: z_factor

 integer :: m0,m1,nband,mband

   m1 = size(A(:,1))

if(present(z_factor)) then

   call zherk('L','N',m1,1,z_factor,A,m1,cmplx(1.0_8,0.0_8,8),C,m1)

else

   call zherk('L','N',m1,1,cmplx(1.0_8,0.0_8,8),A,m1,cmplx(1.0_8,0.0_8,8),C,m1)

endif

end subroutine z_vector_mul_old

!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

subroutine c_vector_mul_old(A,C,z_factor)
 ...
 integer(4) :: m1
   ...
   call cherk('L','N',m1,1,z_factor,A,m1,cmplx(1.0_4,0.0_4,4),C,m1)

   else

   call cherk('L','N',m1,1,cmplx(1.0_4,0.0_4,4),A,m1,cmplx(1.0_4,0.0_4,4),C,m1)

   endif

end subroutine c_vector_mul_old
...
subroutine z_matrix_mul(A,B,C)
 ...
 integer :: m1
   ...
   m1 = size(A(:,1))
   call zgemm('N','N',m1,m1,m1,cmplx(1.0_8,0.0_8,8),A,m1,B,m1,cmplx(0.0_8,0.0_8,8),C,m1)
...
subroutine c_matrix_mul(A,B,C)
 ...
 integer(4) :: m1
   ...
   m1 = size(A(:,1))
   call cgemm('N','N',m1,m1,m1,cmplx(1.0_4,0.0_4,4),A,m1,B,m1,cmplx(0.0_4,0.0_4,4),C,m1)
...
!   n1 = int(j+1-i,4)
!   m2 = int(l+1-k,4)
   !write(*,*) 'Test1',m1,n1,m2
!   allocate(D(m1,n1))
   !call zhemm('L','U',m1,n1,cmplx(1.0_8,0.0_8,8),A,m1,B(:,i:j),m1,cmplx(0.0_8,0.0_8,8),D,m1)
...
!   allocate(D(m1,m1))
   !call zhemm('L','U',m1,m1,cmplx(1.0_8,0.0_8,8),A,m1,B,m1,cmplx(0.0_8,0.0_8,8),D,m1)
!   call zgemm('N','N',m1,m1,m1,cmplx(1.0_8,0.0_8,8),A,m1,B,m1,cmplx(0.0_8,0.0_8,8),D,m1)
!   call zgemm('T','N',m1,m1,m1,cmplx(1.0_8,0.0_8,8),conjg(B),m1,D,m1,cmplx(0.0_8,0.0_8,8),C,m1)
!   deallocate(D)
! end if
...
subroutine c_matrix_elements(A,B,C,i,j,k,l)
 ...
 if(present(i)) then
   ...
   m2 = int(l+1-k,4)
   allocate(D(m1,n1))
   call chemm('L','U',m1,n1,cmplx(1.0_4,0.0_4,4),A,m1,B(:,i:j),m1,cmplx(0.0_4,0.0_4,4),D,m1)
   call cgemm('T','N',m2,n1,m1,cmplx(1.0_4,0.0_4,4),conjg(B(:,k:l)),m1,D,m1,cmplx(0.0_4,0.0_4,4),C,m2)
 else
   m1 = size(A(:,1))
   allocate(D(m1,m1))
   call chemm('L','U',m1,m1,cmplx(1.0_4,0.0_4,4),A,m1,B,m1,cmplx(0.0_4,0.0_4,4),D,m1)
   write(*,*) 'TestTest'
   call cgemm('T','N',m1,m1,m1,cmplx(1.0_4,0.0_4,4),conjg(B),m1,D,m1,cmplx(0.0_4,0.0_4,4),C,m1)
   deallocate(D)
 end if
...
subroutine c_matrix_elements_diag_real(A,B,C)
 ...
 allocate(D(m,m))
 call chemm('L','U',m,m,cmplx(1.0_4,0.0_4,4),A,m,B,m,cmplx(0.0_4,0.0_4,4),D,m)
 C = [(real(dot_product(B(:,i),D(:,i)),4),i=1,m)]
 deallocate(D)
...
subroutine z_matrix_elements_diag_real(A,B,C)
 ...
 C = [(real(dot_product(B(:,i),D(:,i)),8),i=1,m)]
 deallocate(D)
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!%
!% Inverse of Hermitian complex matrix double precision
!%
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
subroutine z_matrix_inverse(A)
  ...
  call ZGETRI(size(A,1), A, size(A,1), ipiv, work, size(A,1), info)

  !call zpotri('U',size(A,1),A,size(A,1),info)

  !call zhptri('U',size(A,1),A,size(A,1),work,info)
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!%
!% Inverse of Hermitian complex matrix single precision
!%
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
subroutine c_matrix_inverse(A)
  ...
  call cpotri('L',size(A,1),A,size(A,1),info)
...
!subroutine timing(string,every)
!
! character(*), optional, intent(in) :: string, every
! integer(8), save :: n = 1
! real(8),    save :: t1
! real(8) :: ts, t
! integer :: td, th, tm
...
!     t = omp_get_wtime()-t1
!   end if
!   td = int(floor(t/(24.0_8*3600.0_8)))
!   t  = t-td*24.0_8*3600.0_8
!   th = int(floor(t/3600.0_8))
!   t  = t-th*3600.0_8
!   tm = (floor(t/60.0_8))
!   ts = t-tm*60.0_8
!   if(present(string)) then
!     if(td>0) then
!       write(*,'(A,A3,I0,A3,I0,A3,I0,A3,F0.3)') trim(string),' d=',td,' h=',th,' m=',tm,' s=',ts
!     else if(th>0) then
!       write(*,'(A,A3,I0,A3,I0,A3,F0.3)') trim(string),' h=',th,' m=',tm,' s=',ts
!     else if(tm>0) then
!       write(*,'(A,A3,I0,A3,F0.3)') trim(string),' m=',tm,' s=',ts
!     else
!         write(*,'(A,A3,F0.3)') trim(string),' s=',ts
!     end if
!   else
!     if(td>0) then
!       write(*,'(A2,I0,A3,I0,A3,I0,A3,F0.3)') 'd=',td,' h=',th,' m=',tm,' s=',ts
!     else if(th>0) then
!       write(*,'(A2,I0,A3,I0,A3,F0.3)') 'h=',th,' m=',tm,' s=',ts
!     else if(tm>0) then
!       write(*,'(A2,I0,A3,F0.3)') 'm=',tm,' s=',ts
!     else
!         write(*,'(A2,F0.3)') 's=',ts
!     end if
!   end if
...
! Imported from FORGE dev for the TightBinding module dependency.
